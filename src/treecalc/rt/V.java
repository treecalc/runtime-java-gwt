...
import treecalc.rt.gwt.CharacterIterator;
import treecalc.rt.gwt.DecimalFormat;
import treecalc.rt.gwt.DecimalFormatSymbols;
import treecalc.rt.gwt.StringCharacterIterator;
...
public abstract class V  {
	...
	public final V bf_format(V formatstr) {
		...
		if (leadingChar=='0')
			decformat = new DecimalFormat(true, symbols);
		else if (leadingChar=='~')
			decformat = new DecimalFormat(false, symbols);
		else if (leadingChar=='*')
		...
		if (preDigitsOffset>=0) {
			decformat.setMinInt(preDigitsValue);
		}

		if (postDigitsOffset<0 && preDigitsOffset<0) {
			decformat.setMaxFrac(33);
		}
		else { 
			decformat.setMinFrac(postDigitsValue);
			decformat.setMaxFrac(postDigitsValue);
		}
		if (thousandSeparator!=CHAR_EMPTY) {
		...
		if (numLeadingChars<=0 && offsetTrailingChars<0) {
			return getInstance(decformat.format(this));
		}

		final String retnum = decformat.format(this);
	...
	/**
	 * GWT-Version of fmod. 
	 * Same as Math.IEEEremainder(this.doubleValue(), divisor.doubleValue())
	 * @param divisor
	 * @return
	 */
	public final V bf_fmod(V divisor) {
		double f1 = this.doubleValue();
		double f2 = divisor.doubleValue();
		double n = Math.round(f1 / f2);
		return getInstance(f1 - f2 * n);
	}
